...
func getOrGen(trackID string, overlay bool) (*bytes.Buffer, error) {
	...
	if overlay {
		...
		c, err := canvas.GetCanvas("spotify:track:" + trackID)
		if err == nil && c != nil && !strings.HasSuffix(c.CanvasUrl, ".mp4") {
			bgimage = c.CanvasUrl
		}
	}
	buf, err = generateImage(pre.CoverArt.Small, pre.TrackName, artistName, bgcolor, bgimage, size)
...
func generateImage(album_art, track_name, artist_name, bg_color string, bg_image string, size []int) (*bytes.Buffer, error) {
	payload := RunPayload{
		Size: size,
		Files: []File{
			{
